...
const News = () => {
    ...
    useEffect(() => {
        if (fetching) {
            ...
            axios.get(`https://www.googleapis.com/books/v1/volumes?q=free-ebooks&download=epub&langRestrict=${lang}&maxResults=${maxResults}&key=${process.env.REACT_APP_API_KEY}&startIndex=${startIndex}`).then(response => {
                setTotalItems(response.data.totalItems);
                setNewBooks([...newBooks, ...response.data.items]);
    ...
    const scrollHandler = (e) => {
        ...
        if (inView && newBooks.length < totalItems) {
            setFetching(true);
        }
    }
    ...
    if (!newBooks) {
        ...
        let new25Books = newBooks.map(book => {
